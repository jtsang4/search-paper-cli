...
7. This skill only recognizes `SEARCH_PAPER_`-prefixed environment variables. Do not infer legacy aliases and do not fuzzy-match variable names. If configuration is missing, explicitly tell the user to add the exact prefixed variable, for example `SEARCH_PAPER_UNPAYWALL_EMAIL=you@example.com`.
