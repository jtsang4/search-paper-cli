...
7. If the wrapper reports missing required environment configuration, stop and tell the user to create or update the skill-local `.env`, then rerun the same command.
8. All other `SEARCH_PAPER_*` variables from `.env.example` are optional.
9. Prefer the CLI's default JSON output unless the user explicitly asks for text output.
